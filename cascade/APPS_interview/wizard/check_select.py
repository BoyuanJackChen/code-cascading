...
all_questions_num = list(range(0,3000))
...
for limit_lines in all_limit_lines:
    for actual_pick_at in all_actual_pick_at:
        for loop in range(num_loops):
            ...
            # print(f"answer_file is {answer_file}")
            ...
            for number in all_questions_num:
                ...
                for answer_dict in answer_data:
                    if answer_dict["number"]==number:
                        answer = answer_dict["answer"]
                        all_answers.append(answer)
                        num_ids += answer_dict["num_ids"]
                        answers_pick_at -= 1
                if len(all_answers) == 0:
                    print(f"Question {number}: No answer")
                    continue
                ...
                for process in processes:
                    process.join(2)  # Kill infinite loops in 2 seconds
                    if process.is_alive():
                        process.terminate()
                        process.join()
